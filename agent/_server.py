...
# Cloud models served via BytePlus Ark — always selectable if ARK_API_KEY is set.
ARK_CATALOG = [
    ("glm-4-7", "GLM-4-7 · BytePlus Ark · Cloud"),
]
...
async def list_models():
    ...
    plus any cloud models available via API keys."""
    ...
    ark_key_present = bool(os.environ.get("ARK_API_KEY"))

    models = [
        {"id": tag, "label": label, "installed": tag in installed}
        for tag, label in SEA_LION_CATALOG
    ] + [
        {"id": tag, "label": label, "installed": ark_key_present}
        for tag, label in ARK_CATALOG
    ]
